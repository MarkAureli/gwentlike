...
- End-of-turn effects fire at the end of each of their owner's turns: front (melee) row left-to-right first, then the back (ranged) row left-to-right. A passed player still gets "phantom turns" between the opponent's turns, so their effects keep firing until the round ends. Examples: Wild Sapling (boosts itself by 1), Drill Sergeant (boosts the unit to its right by 1).
